...
def get_cluster_details(region):
        ...
        try:
            ...
            if response['DBClusters']:
                ...
                if not cluster.get('HttpEndpointEnabled', False):
                    ...
                    print("💡 Run: aws rds modify-db-cluster --db-cluster-identifier stratos-aurora-cluster --enable-http-endpoint --apply-immediately")
                    return None, None
            else:
    ...
    try:
        response = rds_client.describe_db_clusters(
            DBClusterIdentifier='stratos-aurora-cluster'
        )
        
        if not response['DBClusters']:
            print("❌ Aurora cluster 'stratos-aurora-cluster' not found")
            return None, None
        ...
        for secret in secrets['SecretList']:
            if 'aurora' in secret['Name'].lower() and 'stratos' in secret['Name'].lower():
...
def test_data_api(cluster_arn, secret_arn, region):
    ...
    try:
        ...
        if error_code == 'BadRequestException':
            print(f"   ⚠️  Database 'stratos' might not exist or credentials are incorrect")
            ...
            try:
                ...
                print(f"   ✅ Connection successful (but 'stratos' database may not exist)")
                return True
            except:
                pass
        else:
    ...
    try:
        response = client.execute_statement(
            ...
            secretArn=secret_arn,
            database='stratos',
            sql="SELECT pg_database_size('stratos') as size_bytes"
        )
