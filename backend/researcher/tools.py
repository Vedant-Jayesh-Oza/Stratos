"""
Tools for the Stratos Researcher agent
"""
import os
...
STRATOS_API_ENDPOINT = os.getenv("STRATOS_API_ENDPOINT")
STRATOS_API_KEY = os.getenv("STRATOS_API_KEY")
...
def _ingest(document: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with httpx.Client() as client:
        response = client.post(
            STRATOS_API_ENDPOINT,
            json=document,
            headers={"x-api-key": STRATOS_API_KEY},
            timeout=30.0
        )
...
def ingest_financial_document(topic: str, analysis: str) -> Dict[str, Any]:
    """
    Ingest a financial document into the Stratos knowledge base.
    ...
    """
    if not STRATOS_API_ENDPOINT or not STRATOS_API_KEY:
        return {
            "success": False,
            "error": "Stratos API not configured. Running in local mode."
        }
