#!/usr/bin/env python3
"""
Watch CloudWatch logs from all Stratos agents in real-time.
...
LOG_GROUPS = {
    'PLANNER': '/aws/lambda/stratos-planner',
    'TAGGER': '/aws/lambda/stratos-tagger',
    'REPORTER': '/aws/lambda/stratos-reporter',
    'CHARTER': '/aws/lambda/stratos-charter',
    'RETIREMENT': '/aws/lambda/stratos-retirement',
}
...
class AgentLogWatcher:
    ...
    def watch(self, poll_interval: int = 2):
        ...
        print(f"{COLORS['BOLD']}Watching CloudWatch logs for all Stratos agents...{COLORS['RESET']}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Watch CloudWatch logs from all Stratos agents')
    parser.add_argument(
        '--region',
        default='us-east-1',
