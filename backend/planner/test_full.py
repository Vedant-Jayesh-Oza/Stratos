#!/usr/bin/env python3
"""
Run a full end-to-end test of the Stratos agent orchestration.
...
QUEUE_NAME = os.getenv('SQS_QUEUE_NAME', 'stratos-analysis-jobs')
...
def main():
    """Run the full test."""
    print("=" * 70)
    print("🎯 Stratos Agent Orchestration - Full Test")
    print("=" * 70)
    ...
    if job.get('retirement_payload'):
        ...
        if analysis:
            ...
            print(f"  Preview: {preview}")
        if 'success_rate' in ret:
            print(f"  Success Rate: {ret['success_rate']}%")
        if 'projected_value' in ret or 'projected_balance' in ret:
            val = ret.get('projected_value') or ret.get('projected_balance', 0)
            print(f"  Projected Value: ${val:,.0f}")
