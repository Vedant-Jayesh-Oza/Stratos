...
resource "aws_s3_bucket" "vectors" {
  bucket = "stratos-vectors-${data.aws_caller_identity.current.account_id}"
...
resource "aws_iam_role" "lambda_role" {
  name = "stratos-ingest-lambda-role"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name = "stratos-ingest-lambda-policy"
...
resource "aws_cloudwatch_log_group" "lambda_logs" {
  name              = "/aws/lambda/stratos-ingest"
  retention_in_days = 7
...
resource "aws_api_gateway_rest_api" "api" {
  name        = "stratos-api"
  description = "Stratos Financial Planner API"
