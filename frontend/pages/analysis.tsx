...
export default function Analysis() {
            ...
            <div className="glass-card px-8 py-12 text-center">
              ...
              {!fetchingLatest && (
                <button
                  ...
                  className="px-6 py-3 bg-primary/80 text-white rounded-lg hover:bg-primary font-semibold transition-all"
                >
                  Start New Analysis
                </button>
              )}
        ...
        <div className="min-h-screen py-8">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="glass-card px-8 py-12 text-center">
              ...
              <p className="text-gray-400 mb-6">Your analysis is still being processed. Please check back in a few moments.</p>
        ...
        <div className="min-h-screen py-8">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="glass-card px-8 py-12">
              ...
              <p className="text-gray-400 mb-4">The analysis encountered an error and could not be completed.</p>
              {job.error_message && (
      ...
      <div className="prose prose-lg prose-invert max-w-none">
        <ReactMarkdown
          ...
          components={{
            ...
            h2: ({children}) => <h2 className="text-2xl font-semibold mb-3 text-gray-100 mt-6">{children}</h2>,
            h3: ({children}) => <h3 className="text-xl font-medium mb-2 text-gray-200 mt-4">{children}</h3>,
            ...
            th: ({children}) => <th className="p-3 text-left font-semibold border border-white/10 text-gray-300">{children}</th>,
            td: ({children}) => <td className="p-3 border border-white/10 text-gray-400">{children}</td>,
              ...
              <ResponsiveContainer width="100%" height={300}>
                {chartType === 'pie' || chartType === 'donut' ? (
                  <PieChart>
                    ...
                    <Tooltip formatter={(value: number) => `$${value.toLocaleString('en-US')}`} contentStyle={tooltipStyle} />
                  </PieChart>
                  ...
                  <BarChart data={chartData.data}>
                    ...
                    <YAxis tickFormatter={(value) => `$${(value/1000).toFixed(0)}k`} tick={{ fill: '#9ca3af', fontSize: 12 }} />
                    <Tooltip formatter={(value: number) => `$${value.toLocaleString('en-US')}`} contentStyle={tooltipStyle} />
                  ...
                  <LineChart data={chartData.data}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                    <XAxis dataKey={chartData.xKey || "year"} tick={{ fill: '#9ca3af', fontSize: 12 }} />
                    <YAxis tickFormatter={(value) => `$${(value/1000).toFixed(0)}k`} tick={{ fill: '#9ca3af', fontSize: 12 }} />
                    <Tooltip formatter={(value: number) => `$${value.toLocaleString('en-US')}`} contentStyle={tooltipStyle} />
  ...
  const renderRetirement = () => {
    ...
    return (
      <div className="space-y-8">
        {retirementAnalysis && (
          <div className="bg-ai-accent/5 border border-ai-accent/10 rounded-lg p-6">
            <div className="prose prose-lg prose-invert max-w-none">
              <ReactMarkdown
                remarkPlugins={[remarkGfm, remarkBreaks]}
                components={{
                  h2: ({children}) => <h2 className="text-2xl font-semibold mb-3 text-gray-100">{children}</h2>,
                  h3: ({children}) => <h3 className="text-xl font-medium mb-2 text-gray-200">{children}</h3>,
  ...
  const tabs = [
    ...
    { id: 'retirement' as TabType, label: 'Retirement Projection', Icon: Target },
  ];

  return (
    <>
      <Head>
        <title>Analysis - Stratos AI Financial Advisor</title>
      </Head>
      <Layout>
      <div className="min-h-screen py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="glass-card px-8 py-6 mb-8">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => router.push('/advisor-team')}
                className="px-6 py-3 bg-ai-accent/80 text-white rounded-lg hover:bg-ai-accent font-semibold transition-all shadow-lg shadow-ai-accent/20"
              >
                New Analysis
              </button>
            </div>
              ...
              <nav className="flex -mb-px">
                {tabs.map(({ id, label, Icon }) => (
                  <button
                    ...
                    className={`py-3 px-6 border-b-2 font-medium text-sm transition-all flex items-center gap-2 ${
                      activeTab === id
